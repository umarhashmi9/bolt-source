...
${file.content}
    ...
    const deleteProject = useCallback(
        async (projectId: string) => {
            ...
            try {
                ...
                let projDB = dbs.find(db => db.name == projectId);
                if (projDB) {
                    await new Promise<void>((resolve, reject) => {
                        let request = indexedDB.deleteDatabase(projectId);
                        request.onsuccess = () => {
                            resolve();
                        };
                        request.onerror = () => {
    ...
    const reloadCache = useCallback(
        async () => {
            ...
            let projs = await getAllProjects(db);
            setProjects(projs);
            if (id) {
                let project = await getProjectById(db, id);
