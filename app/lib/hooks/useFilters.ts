...
const updateOrderInDB = async (db: IDBDatabase, items: FilterItem[]) => {
    let oldItems: FilterItem[] = await getAllFilters(db);
    let idsInOrder=items.map(item=>item.id);
    await Promise.all(idsInOrder.map(async (id,index)=>{
        let item=oldItems.find(item=>item.id===id);
        if(item){
            item.order=index;
...
export function useFilters() {
    ...
    useEffect(() => {
        ...
        if(!db) return; //wait for the db to initialize
        setIsReady(true);
    ...
    const addFilter = useCallback(
      async (filter: FilterItem) => {
        ...
        let newFilters = await getAllFilters(db);
        newFilters.sort((a, b) => a.order - b.order);
        setFilters(newFilters);
      },
      [db, isReady, filters],
  ...
  const executeFilterChain = useCallback(async (reqbj:filterRequestObject)=>{
      for (let i = 0; i < filters.length; i++) {
        const filter = filters[i];
        let {module,cleanup}=await importModuleFromString(filter.content)
        let resp=await module.filter(reqbj)
        if (resp?.response){
          cleanup()
          return resp
        }
